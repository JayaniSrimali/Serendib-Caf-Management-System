    ...
    if (cartItems.length === 0) {
        return (
            <div className="w-full min-h-screen bg-theme-bg flex flex-col items-center justify-center py-20 px-6 text-center">
                <div className="w-32 h-32 bg-white rounded-full flex items-center justify-center shadow-sm mb-6 text-theme-dark/20">
                    <ShoppingBag size={64} />
                </div>
                <h2 className="text-3xl font-serif font-bold text-theme-dark mb-4">Your cart is empty</h2>
                <p className="text-theme-dark/60 mb-8 max-w-md">Looks like you haven't made your choice yet. Explore our handcrafted menu and find your perfect treat.</p>
    ...
    return (
        <div className="w-full bg-theme-bg min-h-screen py-16 px-6 md:px-12">
            <div className="max-w-[1200px] mx-auto">
                <h2 className="text-4xl font-serif font-bold text-theme-dark mb-10">Your Order summary</h2>

                <div className="flex flex-col lg:flex-row gap-12">
                    {/* Cart Items */}
                    <div className="lg:w-2/3 bg-white rounded-[32px] shadow-sm p-6 md:p-10 border border-theme-dark/5">
                        <div className="space-y-8">
                            {cartItems.map((item) => (
                                <div key={item._id} className="flex flex-col sm:flex-row items-center gap-6 pb-8 border-b border-theme-dark/10 last:border-0 last:pb-0">
                                    ...
                                    <div className="flex-grow flex flex-col justify-between self-stretch py-2 w-full text-center sm:text-left">
                                        <div>
                                            <h3 className="font-serif font-bold text-xl text-theme-dark">{item.name}</h3>
                                            <p className="text-theme-dark/60 text-sm mt-1">{item.category}</p>
                                        </div>
                                        <div className="flex items-center justify-center sm:justify-start gap-6 mt-4 sm:mt-0">
                                            <div className="flex items-center gap-4 bg-theme-bg rounded-full px-4 py-2 border border-theme-dark/10">
                                                <button onClick={() => addToCart(item, -1)} disabled={item.quantity <= 1} className="text-theme-dark hover:text-theme-accent disabled:opacity-30"><Minus size={16} /></button>
                                                <span className="font-bold text-sm w-4 text-center">{item.quantity}</span>
                                                <button onClick={() => addToCart(item, 1)} className="text-theme-dark hover:text-theme-accent"><Plus size={16} /></button>
                                            </div>
                                    ...
                                    <div className="font-bold text-xl text-theme-dark sm:self-end sm:py-2">
                    ...
                    <div className="lg:w-1/3">
                        <div className="bg-theme-dark text-white rounded-[32px] p-8 md:p-10 sticky top-32 shadow-xl">
                            ...
                            <div className="space-y-4 mb-8 text-[#D8C7B9] text-sm">
                            ...
                            <button
                                ...
                                disabled={loading}
                                className="w-full bg-theme-accent text-white py-4 rounded-xl font-bold tracking-widest uppercase hover:bg-white hover:text-theme-dark transition-colors flex items-center justify-center gap-2 group disabled:opacity-50"
                            >
                                {loading ? 'Processing...' : 'Place Order'} <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
                            </button>

                            <div className="mt-8 flex items-center justify-center gap-2 text-[#D8C7B9] text-xs opacity-70">
